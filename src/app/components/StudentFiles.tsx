...
import { Paperclip, GitBranch, FolderOpen } from "lucide-react";

export function StudentFiles({ user }: { user: any }) {
    ...
    return (
      <div className="h-full flex flex-col pt-2 pb-2">
        ...
        <div className="flex-1 min-h-0 relative">
          <WorkspaceFiles projectId={selectedProjectId} user={user} />
        </div>
      </div>
    );
  }
  ...
  if (projects.length === 0) return (
    <div className="text-center py-20">
      ...
      <p className="text-sm text-slate-500 mt-2">You need to be part of a project to access the file repository.</p>
    </div>
  );
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-bold text-slate-800" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>Files & Documents</h2>
        <p className="text-sm text-slate-500 mt-1">Select a project to access its centralized file repository.</p>
      </div>
          ...
          <div key={p.id} onClick={() => setSelectedProjectId(p.id)} className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm hover:shadow-md transition-all cursor-pointer group">
            <div className="flex items-start justify-between mb-4">
              <div className="w-10 h-10 rounded-xl flex items-center justify-center transition-colors" style={{ backgroundColor: p.color ? p.color + "18" : "#f1f5f9" }}>
                <FolderOpen size={18} style={{ color: p.color || "#64748b" }} />
              </div>
            </div>
            ...
            <div className="mt-4 pt-4 border-t border-slate-100 flex items-center justify-between">
              <div className="text-xs font-semibold text-slate-600 flex items-center gap-1.5">
                <Paperclip size={14} className="text-slate-400" /> Open Repository
              </div>
