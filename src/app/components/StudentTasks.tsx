...
export function StudentTasks({ user }: { user: any }) {
    ...
    return (
      <div className="h-full flex flex-col pt-2">
        ...
        <div className="flex-1 min-h-0 bg-slate-50/50 rounded-2xl p-4 border border-slate-100">
          <WorkspaceTasks projectId={selectedProjectId} />
        </div>
      </div>
    );
  }
  ...
  if (projects.length === 0) return (
    <div className="text-center py-20">
      ...
      <p className="text-sm text-slate-500 mt-2">You need to be part of a project to view tasks and milestones.</p>
    </div>
  );
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-bold text-slate-800" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>Tasks & Milestones</h2>
        <p className="text-sm text-slate-500 mt-1">Select a project to manage tasks using the Kanban board.</p>
      </div>
          ...
          <div key={p.id} onClick={() => setSelectedProjectId(p.id)} className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm hover:shadow-md transition-all cursor-pointer group">
            ...
            <div className="mt-4 pt-4 border-t border-slate-100 flex items-center justify-between">
              <div className="text-xs font-semibold text-slate-600 flex items-center gap-1.5">
                <CheckSquare size={14} className="text-slate-400" /> Manage Tasks
              </div>
